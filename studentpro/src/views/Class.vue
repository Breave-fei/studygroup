<template>
  <div class="class">
    <div class="search">
      <span style="font-size: 18px">班级信息：</span>
      <template>
        <el-input
        ...
        <el-table-column fixed="right" label="操作" width="240">
          ...
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="danger"
              @click="handleDelClass(scope.$index, scope.row)"
              >删除</el-button
            >
          </template>
        </el-table-column>
...
<script>
import { getClassPage, delClass } from "@/api/axios/request";
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      console.log("row", row);
      this.classMaskData = row;
      this.flag = true;
    },
    // 删除班级
    handleDelClass(index, row) {
      this.loading = true;
